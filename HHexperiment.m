% Hodgkin-Huxley Neuron Model Simulation with Gaussian-Shaped Pulses
clear all;
...
% Applied current: Gaussian pulses
pulse_duration = 5e-3;  % Pulse duration (s)
pulse_amplitude = 0.22e-9; % Pulse amplitude (A)
pulse_count = 10;       % Number of pulses
pulse_delay = 15e-3;    % Delay between pulses (s)
sigma = 1e-3;           % Standard deviation for Gaussian pulse (s)

% Generate Gaussian pulses
I_ext = zeros(size(time));
for i = 0:(pulse_count-1)
    pulse_center = (i * (pulse_duration + pulse_delay)) + (pulse_duration / 2);
    I_ext = I_ext + pulse_amplitude * exp(-((time - pulse_center).^2) / (2 * sigma^2));
end
...
title('Applied Current (Gaussian Pulses)');
xlabel('Time (ms)');
ylabel('Current (nA)');
grid on;
