% Hodgkin-Huxley Neuron Model Simulation with Baseline Current and Inhibitory Pulses
clear all;
...
h = 0.5;          % Sodium inactivation gating variable
n = 0.35;         % Potassium activation gating variable
...
% Baseline applied current (0.6 nA)
baseline_current = 0.6e-9; % Baseline current (A)
I_ext = baseline_current * ones(size(time));

% Add 10 inhibitory pulses to bring current to zero
pulse_duration = 5e-3;   % Duration of each inhibitory pulse (s)
pulse_count = 10;        % Number of pulses
pulse_delay = 20e-3;     % Delay between pulse onsets (s)

for i = 0:(pulse_count-1)
    start_idx = max(1, ceil((i * pulse_delay) / dt));
    end_idx = min(length(time), start_idx + ceil(pulse_duration / dt) - 1);
    I_ext(start_idx:end_idx) = 0; % Bring current to zero during inhibitory pulses
end
...
title('Applied Current with Inhibitory Pulses');
xlabel('Time (ms)');
ylabel('Current (nA)');
grid on;
